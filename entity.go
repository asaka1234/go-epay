...
// ------------------------------------------------------------
type EPayDepositCancelBackReq struct {
...
type EPayDepositSucceedBackReq struct {
...
type EPayWithdrawReq struct {
	Data []EPayWithdrawData `json:"data" mapstructure:"data"` // 申请sys_no唯一标识 610001
	//这个是EPay给分配的商户id ,sdk来赋值
	//SysNo string `json:"sys_no" mapstructure:"sys_no"` // 申请sys_no唯一标识 610001号
}

type EPayWithdrawData struct {
...
type EPayWithdrawResponse struct {
	Code int    `json:"code"` //200是成功
	Msg  string `json:"msg"`
}

type EPayWithdrawCancelBackReq struct {
	BillNo     string `json:"bill_no" mapstructure:"bill_no"`         // 唯一订单号，商户下单时传过来的order_id
	BillStatus int    `json:"bill_status" mapstructure:"bill_status"` // 订单状态：1=订单已取消；2=订单已激活
	SysNo      string `json:"sys_no" mapstructure:"sys_no"`           // 商户编号
	Sign       string `json:"sign" mapstructure:"sign"`               // 签名，参照验签规范
...
type EPayWithdrawSucceedBackReq struct {
	BillNo string `json:"bill_no" mapstructure:"bill_no"` // 唯一订单号，商户下单时传过来的order_id
...
type EPayWithdrawConfirmReq struct {
	Ids string `json:"ids" mapstructure:"ids"` //确认收款订单列表接口中获取的id，用英文逗号“,”拼接起来
	//这个是EPay给分配的商户id ,sdk来赋值
	//SysNo string `json:"sys_no" mapstructure:"sys_no"` // 申请sys_no唯一标识 610001号
}

type EPayWithdrawConfirmResponse struct {
...
type EPayOrderListRsp struct {
	Code   string              `json:"code"` //
	Msg    string              `json:"msg"`
	Result EPayOrderPageResult `json:"result"`
}

type EPayOrderPageResult struct {
	TotalCount string           `json:"totalCount"` // 总记录数
	TotalPage  int64            `json:"totalPage"`  // 总页数
	Page       int64            `json:"page"`       // 当前页码
	Data       []*EPayOrderData `json:"data"`       // 订单数据列表
}

type EPayOrderData struct {
	ID                       string `json:"id"`
